...
class IngredientPicker extends React.Component {
  ...
  handleSubmit(event) {
    ...
    fetch("https://api.nal.usda.gov/fdc/v1/foods/search?api_key=hLWFG5ApMQiYdHQ5LBIyE3JgCxhhTUOGso4247UG&query=" + encodeURIComponent(this.state.value))
      .then(res => res.json())
      .then(
        (result) => {
          this.setState({
  ...
  handleClick = value => () => {
    ...
    fetch("https://api.nal.usda.gov/fdc/v1/food/" + encodeURIComponent(value) + "?api_key=hLWFG5ApMQiYdHQ5LBIyE3JgCxhhTUOGso4247UG")
      .then(res => res.json())
      .then(
        (result) => {
          this.setState({
